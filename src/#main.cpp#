...
int main(int argc, char* argv[]) {
    ...
    if (!parser_errors.empty()) {
        ...
        std::cerr << "\nblc: " << parser_errors.size() << " error(s) found. compilation stopped.\n";
        return 1;
    }
    ...
    for (const auto& imp : ast->imports) {
        std::string lib_name = imp->value;
        std::string obj_path = "lib/" + lib_name + ".o";
        link_objects.push_back(obj_path);
    }
    ...
    SymbolTable table;
    table = generator.generate(ast.get());
    
    if (opts.verbose || opts.print_symbols) {
        printSymbolTable(table);
    }

    auto symbol_errors = generator.getErrors();
    if (!symbol_errors.empty()) {
        for (const auto& err : symbol_errors) {
            std::cerr << err;
        }
        std::cerr << "\nblc: " << symbol_errors.size() << " error(s) found. compilation stopped.\n";
        return 1;
    }
    ...
    for (const auto& obj : link_objects) {
        cmd += " " + obj;
    }
